...
# Check arguments
if len( argv ) != 2:
    print ("Usage: orca2inp <orca-outfile>")
    print ("")
    print ("Convert orca output to input file for")
    print ("our reinforcment learining project.")
    exit()
...
# parse input
for line in lines:
    if line.startswith( "CARTESIAN COORDINATES (A.U.)" ): # we could take the coordinates in Angstrom aswell would be a minor change
        read = True
        geom = True
        coords = []
        ...
        line = line.strip()
        if line.startswith( "----" ) or line.startswith( "NO" ) or line.startswith( "* core" ):
            continue
        if line:
            line = line.split()
            #coords.append( "%s %s %s %s" %( line[1], line[4], line[5], line[6] ) )
...
# generate input file
out_file = orca_file.split('.')[0] + '.plh'
out = open( out_file, 'w' )
out.write( "[NAMEPLACEHOLDER FORMAT]\n" )
# write frequencies
out.write( "[FREQ]\n" )
for freq in freqs:
    out.write( freq+'\n' )
...
for i in xrange(n_modes):
    ...
    out.write('%16.9f\n' % f)
out.close()
print ("PLACEHOLDER output written to %s" % out_file)
